...
public class UserService(AppDbContext _context) : IUserService
{
    ...
    public async Task<Result> GetAllUsers()
    {
        ...
        return Result.Success(200, JsonSerializer.Serialize(users, _options));
    }
    ...
    public async Task<Result> GetUsersByTime(DateTime? createddate, DateTime? updateddate)
    {
        if (createddate == null || updateddate == null)
        {
            ...
            if (createddate == null)
                return Result.Success(200, JsonSerializer.Serialize(_context.Users.Where(u => u.UpdatedDate == updateddate).Select(u => u.Email).ToListAsync(), _options));
            if  (updateddate == null)
                return Result.Success(200,JsonSerializer.Serialize(_context.Users.Where(u => u.CreatedDate == createddate).Select(u => u.Email).ToListAsync(), _options));
        }

        return Result.Success(200, JsonSerializer.Serialize(_context.Users.Where(u => u.CreatedDate == createddate && u.UpdatedDate == updateddate).Select(u => u.Email).ToListAsync(), _options));
    }

    public async Task<Result> GetMinRegistrationDate()
    {
        return Result.Success(200, JsonSerializer.Serialize(await _context.Users.MinAsync(u => u.CreatedDate)));
    }

    public async Task<Result> GetMaxRegistrationDate()
    {
        return Result.Success(200, JsonSerializer.Serialize(await _context.Users.MaxAsync(u => u.CreatedDate)));
    }

    public async Task<Result> GetSortedUsersByName()
    {
        return Result.Success(200, JsonSerializer.Serialize(await _context.Users.OrderBy(u => u.Name).Select(u => u.Name).ToListAsync(), _options));
    }

    public async Task<Result> GetUsersByGender(Gender gender)
    {
        return Result.Success(200, JsonSerializer.Serialize(await _context.Users.Where(u => u.Gender == gender).Select(u => u.Name).ToListAsync(), _options));
    }
